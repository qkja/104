...
{
public:
  TcpServer(int port, split_t sp, callback_t cb)
      : port_(port), listensock_(-1), epfd_(-1), cb_(cb), PackageSplit(sp)
  {
    ...
    // conn->SetRecver(std::bind(&TcpServer::Accepter, this, std::placeholders::_1)); // 设置读回调
    ...
    // Epoller::AddEvent(epfd_, listensock_, EPOLLIN | EPOLLET);
    // // 添加到这里
    // connections_.insert(std::make_pair(listensock_, new Connection(listensock_, this)));
    // AddConnect(listensock_, EPOLLIN | EPOLLET);
    // 创建就绪fd空间
...
private:
  ...
  //
  std::unordered_map<std::string, callback_t> cbs;
};
