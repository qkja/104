...
namespace Bucket
{
  ...
  public:
    ...
    std::pair<iterator, bool> insert(const T &data)
    {
      ...
      if (_n == _tables.size())
      {
        ...
        std::size_t newSize = GetNextPrime(_tables.size());
  ...
  private:
    std::size_t GetNextPrime(std::size_t prime)
    {
      const int PRIMECOUNT = 28;
      static const std::size_t primeList[PRIMECOUNT] =
          {
